...
/**
 * @title ISCToken
 * @dev Ice Snow Coin - Optimized UUPS Upgradeable ERC20 with security features.
 */
contract ISCToken is 
    Initializable, 
    ERC20Upgradeable, 
    ...
    ReentrancyGuardUpgradeable 
{
    ...
    uint256 public constant TOTAL_SUPPLY = 202_600_000 * 10**18;
    uint256 public constant MAX_TX_PERCENT_LIMIT = 1000; // 10% max tx limit

    // 硬编码地址 / Hardcoded Addresses
    address public constant INITIAL_OWNER = 0xf946A6521D201F2C757562Add139E5635e2a80b3;
    address public constant INITIAL_MARKETING_WALLET = 0xF8A408495941ea30451Da613dC846Dcae47890f0;
    ...
    event GameContractUpdated(address gameContract, bool status);
    event PayForGameItem(address indexed player, uint256 amount);
    event LiquidityPoolSet(address pool);
    event PausedByOwner(address account);
    event UnpausedByOwner(address account);
    event BatchTransfer(address indexed sender, uint256 count);
    event Airdrop(address indexed sender, uint256 count, uint256 amountPer);
    event ExcludedFromMaxTxUpdated(address indexed account, bool excluded);
    event ExcludedFromCooldownUpdated(address indexed account, bool excluded);
    ...
    /**
     * @dev Initializer with hardcoded addresses for security.
     */
    function initialize() public initializer {
        __ERC20_init("Ice Snow Coin", "ISC");
        __ERC20Permit_init("Ice Snow Coin");
        __Pausable_init();
        __Ownable_init(INITIAL_OWNER);
        __UUPSUpgradeable_init();
        __ReentrancyGuard_init();

        marketingWallet = INITIAL_MARKETING_WALLET;
        cooldownTime = 30 seconds;
        cooldownEnabled = true;

        maxTxAmount = TOTAL_SUPPLY * 50 / 10000; // Default 0.5%
        ...
        isExcludedFromCooldown[marketingWallet] = true;
    ...
    function _update(address from, address to, uint256 amount) internal virtual override whenNotPaused {
        ...
        if (cooldownEnabled && isSell && !isExcludedFromCooldown[from]) {
            require(block.timestamp >= lastBuyTime[from] + cooldownTime, "Cooldown: Please wait before selling");
        }
    ...
    // ==================== 游戏支付 ====================
    function payForGameItem(address player, uint256 amount) external whenNotPaused nonReentrant {
        require(isGameContract[msg.sender] || msg.sender == owner(), "Not authorized");
        require(!isBlacklisted[player], "Blacklisted player");
        require(balanceOf(player) >= amount, "Insufficient balance");

        _transfer(player, marketingWallet, amount);
        emit PayForGameItem(player, amount);
    }
    ...
    function batchTransfer(address[] calldata recipients, uint256[] calldata amounts) 
        ...
        nonReentrant 
    {
        require(recipients.length == amounts.length && recipients.length <= 200, "Invalid input");
        ...
        nonReentrant 
    {
        ...
        emit Airdrop(msg.sender, recipients.length, amount);
    }
    ...
    function setLiquidityPool(address _pool) external onlyOwner {
        ...
        liquidityPool = _pool;
        isExcludedFromMaxTx[_pool] = true;
    ...
    function setCooldownTime(uint256 newCooldown) external onlyOwner {
        require(newCooldown <= 300 seconds, "Cooldown too long");
    ...
    function setCooldownEnabled(bool enabled) external onlyOwner {
        ...
        emit CooldownEnabledUpdated(enabled);
    }

    function excludeFromMaxTx(address account, bool excluded) external onlyOwner {
        isExcludedFromMaxTx[account] = excluded;
        emit ExcludedFromMaxTxUpdated(account, excluded);
    }

    function excludeFromCooldown(address account, bool excluded) external onlyOwner {
        isExcludedFromCooldown[account] = excluded;
        emit ExcludedFromCooldownUpdated(account, excluded);
    }

    function setMaxTxAmount(uint256 newMaxTxPercent) external onlyOwner {
        require(newMaxTxPercent <= MAX_TX_PERCENT_LIMIT, "Max tx too high");
        maxTxAmount = TOTAL_SUPPLY * newMaxTxPercent / 10000;
    ...
    function setGameContract(address gameContract, bool status) external onlyOwner {
        isGameContract[gameContract] = status;
        emit GameContractUpdated(gameContract, status);
    }
    ...
    function getCooldownRemaining(address account) external view returns (uint256) {
        if (block.timestamp >= lastBuyTime[account] + cooldownTime) return 0;
        return lastBuyTime[account] + cooldownTime - block.timestamp;
    }
